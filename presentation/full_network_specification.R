...
## png(file = "wheat_network.png", width = 720)
...
hist(posterior, breaks = 100, freq = FALSE, xlim = c(21000, 27000),
     xlab = "Distribution of Wheat Equivalent of Export",
     main = "Comparison of different standardization framework")
posteriorBin = hist(posterior, plot = FALSE, breaks = 100)
abline(v = posteriorBin$mids[which.max(posteriorBin$counts)],
...
biNorm = mvrnorm(n = 10000,
    mu = c(V(standardization.graph)["16"]$weightedExport,
        V(standardization.graph)["21"]$weightedExport),
    Sigma = matrix(c(50000, -4500, -4500, 1000), ncol = 2))
norm1 = rnorm(10000, V(standardization.graph)["16"]$weightedExport,
    50000)
norm2 = rnorm(10000, V(standardization.graph)["21"]$weightedExport,
    1000)
par(mfrow = c(1, 2))
plot(biNorm)
plot(norm1, norm2)
tmp = darchmCopula(u = pnorm(rnorm(10000)), v = pnorm(rnorm(10000)),
    output = "list")
plot(tmp$z)
...
AB1 <- rmvnorm(mean=c(0,0),sig=S,n=1000) #Our gaussian variables
U <- pnorm(AB1) #Now U is uniform - check using hist(U[,1]) or hist(U[,2])
x1 <- qgamma(U[,1],2) #x is gamma distributed
y1 <- qbeta(U[,2],1,2) #y is beta distributed
plot(AB1)
plot(x1,y1) #They correlate!
AB2 <- rmvt(df=c(2,2),sig=S,n=1000) #Our gaussian variables
U <- pt(AB2, df = 2) #Now U is uniform - check using hist(U[,1]) or hist(U[,2])
x2 <- qgamma(U[,1],2) #x is gamma distributed
y2 <- qbeta(U[,2],1,2) #y is beta distributed
plot(AB2)
plot(x2, y2) #They correlate!

par(mfrow = c(2, 2))
S <- matrix(c(1,.8,.8,1),2,2) #Correlation matrix
AB1 <- rmvnorm(mean=c(0,0),sig=S,n=5000) #Our gaussian variables
plot(AB1)
plot(AB1)
AB2 <- rmvt(df=c(2,2),sig=S,n=5000) #Our gaussian variables
U <- pt(AB2, df = 2) #Now U is uniform - check using hist(U[,1]) or hist(U[,2])
x2 <- qnorm(U[,1]) #x is gamma distributed
y2 <- qnorm(U[,2]) #y is beta distributed
plot(AB2)
plot(x2, y2) #They correlate!
